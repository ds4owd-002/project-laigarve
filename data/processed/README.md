...
2.  Methods for processing the data:
    ...
    Within this project, the original Excel file (`Dataset_2017-2021_OSF.xlsx`, sheet 2, 4 and 7) was imported into R and processed to obtain an analysis-ready dataset. Processed datasets are saved as `clinical_processed.csv`, `continuity_processed.csv`, and `index_processed.csv` in the `data/processed` directory.
...
3.  Instrument- or software-specific information needed to interpret the data:
    ...
    -   Data processing and analysis for this project were performed in R, using `readxl` for data import, `janitor` for name cleaning, and `tidyverse` packages (`dplyr`, `tidyr`, `ggplot2`, `purrr`, `readr`, `forcats`, `tibble`) for data manipulation, aggregation, and visualisation.
...
6.  Describe any quality-assurance procedures performed on the data:
    ...
    In the original study, clinical surveillance data were cleaned, anonymised, and cross-checked by the research team in collaboration with local health authorities, with additional linkage to spatial clusters and population estimates as described in the published protocol and main analysis. In this project, quality-assurance steps included verification of variable types, inspection of distributions and missingness patterns, explicit recoding of RDT confirmation and outcome variables, and removal of records missing key identifiers (cluster, year, or admission date) prior to analysis.
